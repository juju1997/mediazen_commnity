...
<body>
	<div class="container">
		<div class="text-center login-title">
			<h1 class="h1">Mediazen Team Community</h1>
		</div>
		<div class="card card-container login-card">
				...
				<form class="form-signin">
						...
						<input name="userId" type="text" id="inputEmail" class="form-control" placeholder="ID를 입력하세요" required autofocus>
						<input name="password" type="password" id="inputPassword" class="form-control" placeholder="비밀번호를 입력하세요" required>
						<button id="login" class="btn btn-lg btn-primary btn-signin" type="submit">로그인</button>
				</form><!-- /form -->
				<div>
					<div>
		...
		$(document).ready(function(){
			...
			$("#login").on("click",function(){
				...
				if((id==null || id=="") || (pw==null || pw=="")){
					...
					$("form").attr("method","POST").attr("action","/user/login.do").submit();
				}
			});
			
			/* find ID */
			$("#forgot-ID").on("click",function(){
				$( location ).attr("href", "/user/findIDGet.do");
			});
			/* find Password */
			$("#forgot-Password").on("click",function(){
				$( location).attr("href", "/user/findPasswordGet.do");
			});
		});
	</script>
